...
class AddressBookWorld {
 ...
 async checkContactStorageCount(expectedCount) {
   const actualCount = await this.page.evaluate(() => JSON.parse(window.localStorage.getItem('task'))
   )
    Array.from(actualCount);
   
   expect(actualCount.lenght).to.be.eq(expectedCount)
 }
}
